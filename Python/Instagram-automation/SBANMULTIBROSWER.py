...
def plusservice1(LINK, data, head):
    global k, txt, send,modplus, contmod1
    global block_msg
    ...
    if "We cannot find record of User ID that you entered." in req.text or "The Given Instagram User ID is Invalid" in req.text:
        ...
        send = False
    if block_msg:
        if "Go to Instagram and confirm" in req.text:
            txt += ", MOD 1 BLOCKED"
    ...
    if "We could not process your request. Please try again later." in req.text:
        ...
        send = False
    if "The decision to disable your account can&#039;t be reviewed." in req.text:
        txt+= ", BLOCCO DEFINITIVO"
        message("@" + user[k] + ' BLOCK DEFINITIVO.')
        send = False
    if "You may want to slow down or stop to avoid a restriction on your account. We limit how often you can post, " \
       "comment or do other things in a given amount of time to help protect the community from spam." in req.text:
        ...
        if modplus == 1:
            ...
            contmod1 += 1
            if contmod1 == 3:
                message_auth(str(user[k]) + ";" + str(nomis[k]) + ";" + str(
                    maill[k]) + ";" + phonis[k] + ";" + text[k] + ";" + msg + ";" + nomepc + ";" + logins)
                contmod1 = 0
        else:
            mode = "mod 2"
        ...
        send = False
    if "recaptcha" in req.text:
        txt += ", MAYBE CAPTCHA DETECTED"
        #message("@" + user[k] + " POSSIBILE CAPTCHA")
        send = False
...
def message_auth(testo):
    global nomepc
    if logins.lower() == "si":
        try:
            client.send_message("@manuelyoo_bot", message=testo)
        except:
            pass
...
def mod1selenium():
    ...
    try:
        driver = chrome()
        driver.get(
            "https://www.google.com/search?q=instagram+contact+606967319425038&rlz=1C1ONGR_itIT1022IT1022&oq=instagram+contact+606967319425038&aqs=chrome.0.69i59j0i546l4j69i60l3.335j0j9&sourceid=chrome&ie=UTF-8")
        ...
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
             "/html/body/div/div/div[3]/div/div[2]/div/form/div[2]/input"))).send_keys(nomis[k])
        sleep(2)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
             "/html/body/div/div/div[3]/div/div[2]/div/form/div[3]/input"))).send_keys(maill[k])
        sleep(2)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
             "/html/body/div/div/div[3]/div/div[2]/div/form/div[4]/input"))).send_keys(user[k])
        sleep(2)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
             "/html/body/div/div/div[3]/div/div[2]/div/form/div[5]/input"))).send_keys(phonis[k])
        sleep(2)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
             "/html/body/div/div/div[3]/div/div[2]/div/form/div[6]/textarea"))).send_keys(text[k])
        sleep(2)
        WebDriverWait(driver, 20).until(EC.element_to_be_clickable(
            (By.XPATH,
        ...
        try:
            ...
            send = False
            txt+=", blocco indefinito mod1"
    except Exception as e:
        print(e)
        send = False
        txt += ", Mod1 errore generico"
    return  send
        ...
        while True:
            ...
            while k < righe:
                ...
                if str(modd1[k]).lower() == "x" or modd1[k] == 'X' or str(modd1[k]).lower() == str("xx").lower() or str(
                        modd1[k]).lower() == str("xxx").lower() or str(
                        modd1[k]).lower() == str("xxxx").lower() or str(
                        modd1[k]).lower() == str("xxxxx").lower():
                    try:
                        ...
                        if str(modd1[k]).lower() == str("xx").lower():
                            ...
                            send = mod8plus()
                        elif str(modd1[k]).lower() == str("xxxx").lower():
                            send = mod1selenium()
                        elif str(modd1[k]).lower() == str("xxxxx").lower():
                            send = modfb1plus()
                        else:
                            send = mod1plus()
                        if send == False:
...
if __name__ == "__main__":
    ...
    print(Fore.LIGHTBLUE_EX + '                       SBANYOO ' + Style.RESET_ALL)
    print(Fore.BLUE + "------------------------------------------------------" + Style.RESET_ALL)
    global insta, client, nomefile, contmod1
